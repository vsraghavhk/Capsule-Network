...
def train(dataset, ckpt=None, output=None):
    ...
    def preprocessing_function(image):
        ...
        image = image * 255       # Commented out defined preprocessing, add Edge detection
        
        #                     Existing pre-processing by T. Neveu                    
        ...
        image = cv2.Canny(np.uint8(gray), 150, 450) # Arbitrary threshold value
            # Converts images to edgedetected images,   1 channel 256 level
        image = image.reshape((image.shape[0], image.shape[1], 1))
        # _, binedge = cv2.threshold(cv2.Canny(gray, 150, 450),127,375,cv2.THRESH_BINARY) 
            # cv2.threshold(img,127,375,cv2.THRESH_BINARY) 
                # converts edged, 1 channel 256 level to edged 1 channel binary level image
        """
    ...
    # Init model
    model = ModelIdol("Idols", output_folder=output)
    if ckpt is None:
        model.init()
    else:
        model.load(ckpt)
    ...
    while True:
        ...
        if b % 500 == 0: # Test the model on all the validation
